...
class RolloutBuffer:
    ...
    def __init__(
        ...
        # critic 使用的观测，允许比 policy observation 更大
    ...
    def _episode_tensor_fields(self):
        fields = {
            "obs": self.obs,
            ...
            "reward_reg": self.reward_reg,
            "reward_sil": self.reward_sil,
            "reward_total": self.reward_total,
            "skill_ids": self.skill_ids,
            "episode_ids": self.episode_ids,
            "command_speeds": self.command_speeds,
            "dones": self.dones,
            "terminateds": self.terminateds,
            "truncateds": self.truncateds,
        }
        if self.imitation_obs is not None:
            fields["imitation_obs"] = self.imitation_obs
        if self.command_onehots is not None:
            fields["command_onehots"] = self.command_onehots
        return fields
    ...
    def get_minibatches(self, batch_size, shuffle=True):
        ...
        batch = {
            ...
            "critic_obs": self._flatten_valid(self.critic_obs),
    ...
    def extract_episodes(self):
        ...
        for env_id in range(self.num_envs):
            ...
            for t in range(valid_steps):
                ...
                current["critic_obs"].append(self.critic_obs[t, env_id].clone())
    ...
    def extract_episodes_tensor(self):
        """
        从 rollout buffer 中按环境维度提取 episode 序列的 tensor 视图。

        返回:
        - episodes: list[dict[str, torch.Tensor]]

        与 `extract_episodes()` 保持同样的切分逻辑：
        - 遇到 done 就切分一个完整 episode
        - rollout 尾部未 done 的残余片段也会保留

        但这里避免了逐步 append / clone，而是直接对 [time, env, feature]
        张量按区间切片，作为后续 DTW / discriminator 的高吞吐快路径。
        """
        valid_steps = self._valid_steps()
        if valid_steps == 0:
            return []
        ...
        field_tensors = {
            key: value[:valid_steps]
            for key, value in self._episode_tensor_fields().items()
        }

        episodes = []
        for env_id in range(self.num_envs):
            done_mask = field_tensors["dones"][:, env_id, 0] > 0.5
            done_indices = torch.nonzero(done_mask, as_tuple=False).reshape(-1).tolist()
            start = 0

            for done_index in done_indices:
                end = int(done_index) + 1
                episodes.append(
                    {
                        key: tensor[start:end, env_id].clone()
                        for key, tensor in field_tensors.items()
                    }
                )
                start = end

            if start < valid_steps:
                tail_episode = {
                    key: tensor[start:valid_steps, env_id].clone()
                    for key, tensor in field_tensors.items()
                }
                if tail_episode["obs"].shape[0] > 0:
                    episodes.append(tail_episode)
        return episodes
